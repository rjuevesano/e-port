<?php
  ...
  if (isset($_GET['notification_id'])) {
    ...
    $sql_notification_update = "update notification set is_read=true where notification_id=$notificationId";
    $conn->query($sql_notification_update);
  }
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action']) && $_POST['action'] == 'addmessage') {
      ...
      $sql_check = "select count(*) as total from message where user_id_client=$user_id and user_id_supplier=$current_user_id and is_main=true";
      $result_check = $conn->query($sql_check);
      $total = $result_check->num_rows ? $result_check->fetch_assoc()['total'] : 0;

      if ($total) {
        $sql_update = "update message set updated=now() where user_id_client=$user_id and user_id_supplier=$current_user_id and is_main=true";
        $conn->query($sql_update);

        $sql = "insert into message (user_id_client, user_id_supplier, is_main, sender, text) values('$user_id', '$current_user_id', false, 'SUPPLIER', '$message')";
        $conn->query($sql);
      } else {
        $sql = "insert into message (user_id_client, user_id_supplier, is_main, sender, text) values('$user_id', '$current_user_id', true, 'SUPPLIER', '$message')";
        $conn->query($sql);
      }
                ...
                <div class="inbox_chat">
                  <?php
                    ...
                    while($row = $result->fetch_assoc()) {
                      ...
                      if ($row_user_client['avatar']) {
                        $avatar = "../client/uploads/".$row_user_client['avatar'];
                      }
                  ...
                  ?>
                  <div class="chat_list <?php echo (isset($_GET['client_id']) && $user_id_client == $_GET['client_id']) ? 'active_chat': ''  ?>">
                    <a href="messages.php?client_id=<?php echo $user_id_client ?>" style="text-decoration: none;">
                      <div class="chat_people">
                        ...
                        <div class="chat_ib">
                          <h5>
                            <?php echo $row_user_client['firstname']." ".$row_user_client['lastname'] ?>
                            <span class="chat_date"><?php echo getDateTimeDifferenceString($row['updated']) ?></span>
                          </h5>
                ...
                <div class="msg_history">
                  <?php
                    ...
                    while($row_messages=$result_messages->fetch_assoc()) {
                      ...
                      $avatar = $row_user_client['avatar'] ? "../client/uploads/".$row_user_client['avatar'] : '../../img/undraw_profile.svg';
                      
                      if ($row_messages['sender']=='SUPPLIER') {
                  ?>
                  <div class="outgoing_msg">
                    <div class="sent_msg">
                      <p><?php echo $row_messages['text'] ?></p>
                      <span class="time_date"><?php echo getDateTimeDifferenceString($row_messages['created']) ?></span>
                    </div>
                  </div>
                  <?php } else { ?>
